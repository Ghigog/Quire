# ADR-0006 — Character voices are generated, not assigned
...
Before designing around "generate", QUI-034 asked whether the engine we already committed to
...
runtime, nothing added to the 450 MB footprint. Full numbers in QUI-034's Worklog.
...
- **Description-to-voice models** (Parler-TTS, VoiceLDM), which take an English description
  ...
  SLM (ADR-0005), with hopeless throughput on a Snapdragon 750G. Revisit only if the device
  changes.
...
- **Gender may stop being the right abstraction.** QUI-033 exists to raise gender coverage
  ...
  and QUI-033's framing should be revisited before it is worked.
- **Pace comes free.** The model takes `length_scale`, so speaking rate is per-character at
  no cost.

## What is not established

**Nothing in QUI-034 was listened to.** F0 and duration prove the audio is well-formed and
...
rather than phoneme count; their null rows in QUI-034 mean "the probe cannot see it", not
"nothing happened".
