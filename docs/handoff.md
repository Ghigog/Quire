...
| --- | --- | --- | --- |
| **A** | Who is in this book? | one pass | Heuristics, 84.8% precision (QUI-032) |
| **B** | Who says line 4,412? | ~3,000 unresolved quotations | Heuristics + turn-taking; the real gap |
| **C** | What does each character sound like? | ~10 judgements | Not built — ADR-0006 |
...
- **C does not depend on B.** To design Geralt's voice you need *some* lines you are
  ...
  real.
- **Only B is expensive**, which is what makes on-device viable at all (ADR-0005).
...
| All book analysis runs on-device | ADR-0005 | CLAUDE.md §8 is a hard rule; cloud was costed, not hand-waved |
| Voices are generated, not assigned | ADR-0006 | A voice is 512 editable floats; assignment throws away what the analysis knew |
...
1. **Does a generated voice sound like a person?** The whole of ADR-0006 rests on this and
...
2. **Do Scots phonemes through an `en-US`-trained model sound Scottish, or broken?**
   ...
   wrote in the dialogue" and ADR-0006's accent claim needs striking.
...
6. **Is gender still the right abstraction?** QUI-033 wants to raise gender coverage because
   `Casting` uses gender to pick a pool. Under ADR-0006 gender is one input to a pitch and
   timbre target, not a selector. **Revisit QUI-033's framing before working it** — it may be
...
Read in this order: `CLAUDE.md` → this file → ADR-0005 and ADR-0006 → the Worklogs of
QUI-032 and QUI-034. Then:

```bash
gradle test                                    # whole JVM suite, seconds
cd spike/pipeline && gradle installDist        # then: cast <pdnc>/data/*   (see QUI-032)
