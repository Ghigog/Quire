 ...
 *
 * It is the number every other candidate has to beat, and it is also the floor — Tier 1 runs
 * before any model whatever the bake-off decides, so a candidate is really being asked what
 * it adds to this.
 *
 * Both rules that trade precision for coverage are switchable, because the only fair way to
 * say what a rule bought is to score it against its own absence on identical text.
 */
class Tier1Candidate(
    ...
    override fun answer(paragraphs: List<ParagraphUnit>, questions: List<Question>): Map<String, Answer> {
        val cast = Tier1.bootstrapRoster(paragraphs)
        val manifest = Roster.manifest(cast, bookId = "bakeoff", generatedAt = 0L)
        val heuristic = Heuristic(manifest, pronouns = pronouns, actionBeats = actionBeats)

        // Attribute the book once, then index the spans by paragraph, so a question is
        // answered by a lookup rather than by re-running the rules per quotation.
        val byParagraph = HashMap<Int, List<Span>>(paragraphs.size)
        ...
        return questions.associate { question ->
            ...
            // No quoted span covers the gold quotation: the segmenter did not see speech
